...
public class MatchService{
    ...
    private boolean matchPersons(List<MatchDTO> filteredByPersons){
        filteredByPersons.forEach( m-> System.out.println("들어온 회원:"+m.getMember().getName()));
        ...
        if(filteredByPersons.size()>=1){
            ...
            if(agreementFlag==1){
                //매치 >> 매치웨이트 처리
                for( int i=0; i<scatteredList.size();i++)convert(modelMapper.map(scatteredList.get(i),MatchEntity.class),i<scatteredList.size()?"A":"B");
                return true;
...
//        } //여기 로직에서 실제 포인트 차감
    ...
    public List<MatchDTO> getMatchList(Long userId) {
        ...
        List<MatchDTO> agreeList = agreeEntityList.stream()
